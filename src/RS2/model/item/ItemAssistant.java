...
public class ItemAssistant {
            ...
            if (c.equipment.getItemIds()[i] > -1) {
				for (int j = 0; j < Settings.ITEM_LIMIT; j++) {
					if (GameEngine.itemHandler.ItemList[j] != null) {
                        if (GameEngine.itemHandler.ItemList[j].itemId == c.equipment.getItemIds()[i]) {
	...
	public boolean wearItem(int wearID, int slot) {
		synchronized (c) {
            ...
            if (c.inventory.getItemIds()[slot] == (wearID + 1)) {
		...
		for (int i = 0; i < Settings.ITEM_LIMIT; i++) {
			if (GameEngine.itemHandler.ItemList[i] != null) {
				if (GameEngine.itemHandler.ItemList[i].itemName == NotedName) {
					if (GameEngine.itemHandler.ItemList[i].itemDescription
