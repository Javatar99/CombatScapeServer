...
public class PlayerAssistant {
    ...
    public void potionPoisonHeal(int itemId, int itemSlot, int newItemId, int healType) {
        c.attackTimer = c.getCombat().getAttackDelay(c.getItems().getItemName(c.equipment.getItemIds()[c.playerWeapon]).toLowerCase());
        if (c.duelRule[5]) {
    ...
    public int antiFire() {
        ...
        if (c.antiFirePot)
            toReturn++;
        if (c.equipment.getItemIds()[c.playerShield] == 1540 || c.prayerActive[12] || c.equipment.getItemIds()[c.playerShield] == 11284)
            toReturn++;
        return toReturn;
    }
    ...
    public int getWearingAmount() {
        int count = 0;
        for (int j = 0; j < c.equipment.getItemIds().length; j++) {
            if (c.equipment.getItemIds()[j] > 0)
                count++;
        }
        return count;
    }
