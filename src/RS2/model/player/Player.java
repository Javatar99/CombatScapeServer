...
public abstract class Player {
    ...
    public static final int maxPlayerListSize = Settings.MAX_PLAYERS;
    ...
    dtOption = false, dtOption2 = false, doricOption = false,
            ...
            caPlayerTalk1 = false, disableAttEvt = false,
    ...
    public long lastPlayerMove, lastPoison, lastPoisonSip, poisonImmune,
            lastSpear, lastProtItem, lastVeng, lastYell,
    ...
    public OptionDialogue currentOptionDialogue;
    ...
    public byte cachedPropertiesBitmap[] = new byte[(Settings.MAX_PLAYERS + 7) >> 3];
    ...
    public int[] damageTaken = new int[Settings.MAX_PLAYERS];
    ...
    public Player(int _playerId) {
        ...
        for (int i = 0; i < inventory.getItemIds().length; i++) {
            inventory.getItemIds()[i] = 0;
        }
        for (int i = 0; i < inventory.getItemAmounts().length; i++) {
        ...
        equipment.getItemIds()[playerHat] = -1;
        equipment.getItemIds()[playerCape] = -1;
        equipment.getItemIds()[playerAmulet] = -1;
        equipment.getItemIds()[playerChest] = -1;
        equipment.getItemIds()[playerShield] = -1;
        equipment.getItemIds()[playerLegs] = -1;
        equipment.getItemIds()[playerHands] = -1;
        equipment.getItemIds()[playerFeet] = -1;
        equipment.getItemIds()[playerRing] = -1;
        equipment.getItemIds()[playerArrows] = -1;
        equipment.getItemIds()[playerWeapon] = -1;

        heightLevel = 0;

        teleportToX = Settings.START_LOCATION_X;
        teleportToY = Settings.START_LOCATION_Y;
    ...
    public boolean fullVoidRange() {
        return equipment.getItemIds()[playerHat] == 11664
                && equipment.getItemIds()[playerLegs] == 8840
                && equipment.getItemIds()[playerChest] == 8839
                && equipment.getItemIds()[playerHands] == 8842;
    }

    public boolean fullVoidMage() {
        return equipment.getItemIds()[playerHat] == 11663
                && equipment.getItemIds()[playerLegs] == 8840
                && equipment.getItemIds()[playerChest] == 8839
                && equipment.getItemIds()[playerHands] == 8842;
    }
    ...
    public boolean fullVoidMelee() {
        return equipment.getItemIds()[playerHat] == 11665
                && equipment.getItemIds()[playerLegs] == 8840
                && equipment.getItemIds()[playerChest] == 8839
                && equipment.getItemIds()[playerHands] == 8842;
    }
    ...
    protected void appendPlayerAppearance(Stream str) {
        ...
        if (equipment.getItemIds()[playerHat] > 1) {
            playerProps.writeWord(0x200 + equipment.getItemIds()[playerHat]);
        } else {
            playerProps.writeByte(0);
        }

        if (equipment.getItemIds()[playerCape] > 1) {
            playerProps.writeWord(0x200 + equipment.getItemIds()[playerCape]);
        } else {
            playerProps.writeByte(0);
        }

        if (equipment.getItemIds()[playerAmulet] > 1) {
            playerProps.writeWord(0x200 + equipment.getItemIds()[playerAmulet]);
        } else {
            playerProps.writeByte(0);
        }

        if (equipment.getItemIds()[playerWeapon] > 1) {
            playerProps.writeWord(0x200 + equipment.getItemIds()[playerWeapon]);
        } else {
            playerProps.writeByte(0);
        }

        if (equipment.getItemIds()[playerChest] > 1) {
            playerProps.writeWord(0x200 + equipment.getItemIds()[playerChest]);
        } else {
            playerProps.writeWord(0x100 + playerAppearance[2]);
        }

        if (equipment.getItemIds()[playerShield] > 1) {
            playerProps.writeWord(0x200 + equipment.getItemIds()[playerShield]);
        } else {
            playerProps.writeByte(0);
        }

        if (!Item.isFullBody(equipment.getItemIds()[playerChest])) {
            playerProps.writeWord(0x100 + playerAppearance[3]);
        } else {
            playerProps.writeByte(0);
        }

        if (equipment.getItemIds()[playerLegs] > 1) {
            playerProps.writeWord(0x200 + equipment.getItemIds()[playerLegs]);
        } else {
            playerProps.writeWord(0x100 + playerAppearance[5]);
        }

        if (!Item.isFullHelm(equipment.getItemIds()[playerHat])
                && !Item.isFullMask(equipment.getItemIds()[playerHat])) {
            playerProps.writeWord(0x100 + playerAppearance[1]);
        } else {
            playerProps.writeByte(0);
        }

        if (equipment.getItemIds()[playerHands] > 1) {
            playerProps.writeWord(0x200 + equipment.getItemIds()[playerHands]);
        } else {
            playerProps.writeWord(0x100 + playerAppearance[4]);
        }

        if (equipment.getItemIds()[playerFeet] > 1) {
            playerProps.writeWord(0x200 + equipment.getItemIds()[playerFeet]);
        } else {
        ...
        int mag = (int) (skills.getSkill(6).getActualLevel() * 1.5);
        int ran = (int) (skills.getSkill(4).getActualLevel() * 1.5);
        int attstr = (int) ((double) skills.getSkill(0).getActualLevel() + (double) skills.getSkill(2).getActualLevel());

        combatLevel = 0;
        if (ran > attstr) {
            combatLevel = (int) ((skills.getSkill(1).getActualLevel() * 0.25)
                    + (skills.getSkill(3).getActualLevel() * 0.25)
                    + (skills.getSkill(5).getActualLevel() * 0.125) + (skills.getSkill(4).getActualLevel() * 0.4875));
        } else if (mag > attstr) {
            combatLevel = (int) ((skills.getSkill(1).getActualLevel() * 0.25)
                    + (skills.getSkill(3).getActualLevel() * 0.25)
                    + (skills.getSkill(5).getActualLevel() * 0.125) + (skills.getSkill(6).getActualLevel() * 0.4875));
        } else {
            combatLevel = (int) ((skills.getSkill(1).getActualLevel() * 0.25)
                    + (skills.getSkill(3).getActualLevel() * 0.25)
                    + (skills.getSkill(5).getActualLevel() * 0.125)
                    + (skills.getSkill(0).getActualLevel() * 0.325) + (skills.getSkill(2).getActualLevel() * 0.325));
        }
    ...
    public boolean wearing2h() {
        Client c = (Client) this;
        String s = c.getItems().getItemName(c.equipment.getItemIds()[c.playerWeapon]);
        if (s.contains("2h"))
            return true;
    ...
    protected void appendHitUpdate(Stream str) {
        ...
        if (skills.getSkill(3).getCurrentLevel() <= 0) {
            skills.getSkill(3).setCurrentLevel(0);
            isDead = true;
        }
        str.writeByteC(skills.getSkill(3).getCurrentLevel()); // Their current hp, for HP bar
        str.writeByte(skills.getSkill(3).getActualLevel()); // Their max hp, for HP bar
    ...
    protected void appendHitUpdate2(Stream str) {
        ...
        str.writeByte(skills.getSkill(3).getCurrentLevel()); // Their current hp, for HP bar
        str.writeByteC(skills.getSkill(3).getActualLevel()); // Their max hp, for HP bar
