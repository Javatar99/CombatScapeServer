		...
		switch(packetType) {
			...
			**/
			...
			boolean usingArrows = false;
                ...
                if (c.equipment.getItemIds()[c.playerWeapon] >= 4214 && c.equipment.getItemIds()[c.playerWeapon] <= 4223)
				usingBow = true;
			...
			if(usingOtherRangeWeapons && c.goodDistance(c.getX(), c.getY(), NPCHandler.npcs[c.npcIndex].getX(), NPCHandler.npcs[c.npcIndex].getY(), 4)) {
                ...
                if (!usingCross && !usingArrows && usingBow && c.equipment.getItemIds()[c.playerWeapon] < 4212 && c.equipment.getItemIds()[c.playerWeapon] > 4223 && !usingCross) {
				c.sendMessage("You have run out of arrows!");
				break;
			}
                if (c.getCombat().correctBowAndArrows() < c.equipment.getItemIds()[c.playerArrows] && Settings.CORRECT_ARROWS && usingBow && !c.getCombat().usingCrystalBow() && c.equipment.getItemIds()[c.playerWeapon] != 9185) {
                    c.sendMessage("You can't use " + c.getItems().getItemName(c.equipment.getItemIds()[c.playerArrows]).toLowerCase() + "s with a " + c.getItems().getItemName(c.equipment.getItemIds()[c.playerWeapon]).toLowerCase() + ".");
				c.stopMovement();
				c.getCombat().resetPlayerAttack();
				return;
			}
                if (c.equipment.getItemIds()[c.playerWeapon] == 9185 && !c.getCombat().properBolts()) {
