	...
	**/
	...
	public void openDuel() {
		...
		for (int i = 0; i < c.equipment.getItemIds().length; i++) {
			sendDuelEquipment(c.equipment.getItemIds()[i], c.equipment.getItemAmounts()[i], i);
		}
	...
	public void refreshDuelScreen() {
		synchronized(c) {
			...
			for(GameItem item : stakedItems) {
				...
				if (item.id > Settings.ITEM_LIMIT || item.id < 0) {
					item.id = Settings.ITEM_LIMIT;
				}
				c.getOutStream().writeWordBigEndianA(item.id + 1);
	...
	public void startDuel() {
		...
		if(c.duelRule[11]) {
			c.getItems().removeItem(c.equipment.getItemIds()[0], 0);
		}
		if(c.duelRule[12]) {
			c.getItems().removeItem(c.equipment.getItemIds()[1], 1);
		}
		if(c.duelRule[13]) {
			c.getItems().removeItem(c.equipment.getItemIds()[2], 2);
		}
		if(c.duelRule[14]) {
			c.getItems().removeItem(c.equipment.getItemIds()[3], 3);
		}
		if(c.duelRule[15]) {
			c.getItems().removeItem(c.equipment.getItemIds()[4], 4);
		}
		if(c.duelRule[16]) {
			c.getItems().removeItem(c.equipment.getItemIds()[5], 5);
		}
		if(c.duelRule[17]) {
			c.getItems().removeItem(c.equipment.getItemIds()[7], 7);
		}
		if(c.duelRule[18]) {
			c.getItems().removeItem(c.equipment.getItemIds()[9], 9);
		}
		if(c.duelRule[19]) {
			c.getItems().removeItem(c.equipment.getItemIds()[10], 10);
		}
		if(c.duelRule[20]) {
			c.getItems().removeItem(c.equipment.getItemIds()[12], 12);
		}
		if(c.duelRule[21]) {
			c.getItems().removeItem(c.equipment.getItemIds()[13], 13);
		}		
		c.duelStatus = 5;
		c.getPA().removeAllWindows();
		c.specAmount = 10;
		c.getItems().addSpecialBar(c.equipment.getItemIds()[c.playerWeapon]);
			...
			if(item.id > 0 && item.amount > 0) {
				if(Item.itemStackable[item.id]) {
					if(!c.getItems().addItem(item.id, item.amount)) {
						GameEngine.itemHandler.createGroundItem(c, item.id, c.getX(), c.getY(), item.amount, c.getId());
					}
				} else {
					int amount = item.amount;
					for(int a = 1; a <= amount; a++) {
						if(!c.getItems().addItem(item.id, 1)) {
							GameEngine.itemHandler.createGroundItem(c, item.id, c.getX(), c.getY(), 1, c.getId());
						}
					}
				}
			}	
	...
	public void selectRule(int i) { // rules
		...
		if(i >= 11 && c.duelSlot > -1) {
			if (c.equipment.getItemIds()[c.duelSlot] > 0) {
				if(!c.duelRule[i]) {
					c.duelSpaceReq++;	
				} else {
					c.duelSpaceReq--;
			...
			if (o.equipment.getItemIds()[o.duelSlot] > 0) {
				if(!o.duelRule[i]) {
					o.duelSpaceReq++;
				} else {
					o.duelSpaceReq--;
